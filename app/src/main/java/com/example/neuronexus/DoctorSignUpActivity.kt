...
class DoctorSignUpActivity : AppCompatActivity() {
    ...
    // Variables to store image URIs
    private var profileImageUri: Uri? = null
    private var licenseImageUri: Uri? = null

    // Temp URI for the camera
    ...
    private val profileCameraLauncher = registerForActivityResult(ActivityResultContracts.TakePicture()) { success ->
        // Restore the temp uri if the activity was killed
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState != null) {
            // Restore URIs from the saved bundle
            ...
            // Re-display images if they existed
    ...
    // --- SAVE STATE (The Fix) ---
    // This runs just before the Activity is destroyed (e.g. when Camera opens)
    ...
    private fun openCamera(isProfile: Boolean) {
        ...
        tempCameraUri = uri // Save to global variable so we can save it in onSaveInstanceState
    ...
    private fun validateAndRegister() {
        ...
        // Removed strict check for licenseImageUri for testing if needed, but keeping for production
        if (licenseImageUri == null) {
