package com.example.neuronexus.repository
...
//    REGISTER PATIENT
    ...
    /**
     * HELPER: Uploads image and returns URL.
     * Logic: If Uri is null (User didn't pick image), return "" string immediately.
     */
    private fun uploadImage(
        ...
        if (uri == null) {
            onComplete("") // Return empty string for default/no image
            return
        }
        ...
        storageRef.putFile(uri)
            ...
            .addOnFailureListener {
                onComplete("") // If upload fails, continue with empty string (don't block registration)
            }
    }
    ...
    fun loginUser(email: String, pass: String, callback: LoginCallback) {
        auth.signInWithEmailAndPassword(email, pass)
            ...
            .addOnSuccessListener { authResult ->
                ...
                // Fetch the 'users' node to check role and status
                db.child("users").child(uid).get()
                    .addOnSuccessListener { snapshot ->
                        if (snapshot.exists()) {
                            // Convert JSON snapshot to User object
                            val user = snapshot.getValue(User::class.java)
                            if (user != null) {
                                // Pass the full User object (containing role) back to Activity
